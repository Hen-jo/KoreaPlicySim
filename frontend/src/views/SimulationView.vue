    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          <span class="step-num">단계 2/5</span>
          <span class="step-name">유권자 모델링</span>
        </div>
...
const handleNextStep = (params = {}) => {
  addLog('Entered Step 3: Start Simulation')
  
  // 记录模拟轮数配置
  if (params.maxRounds) {
    addLog(`Custom max rounds: ${params.maxRounds}`)
  } else {
    addLog('Using auto-generated round count')
  }
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Detected a running simulation. Stopping it...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
