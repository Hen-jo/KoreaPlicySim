    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a new simulation instance and load the base world configuration.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Use the graph context to extract entities and relations, initialize simulated individuals, and assign them distinct behavior and memory based on the source material.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Linked Seed Topics</span>
            </div>
          </div>
      ...
      <!-- Step 03: Generate Dual-Platform Configuration -->
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">03</span>
            <span class="step-title">Generate Dual-Platform Configuration</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 2" class="badge success">Completed</span>
            <span v-else-if="phase === 2" class="badge processing">Running</span>
            <span v-else class="badge pending">Pending</span>
          </div>
        </div>
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM configures time flow, recommendation dynamics, active hours, posting frequency, and event triggers based on the scenario and source material.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                <div class="config-item">
                  <span class="config-item-label">Simulation Length</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Recommendation Configuration</span>
              </div>
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Platform 1: Public Feed</span>
                  </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          ...
          <div class="step-status">
            ...
            <span v-else-if="phase === 3" class="badge processing">Running</span>
            <span v-else class="badge pending">Pending</span>
          </div>
        </div>
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generates seed activation events and hot topics from the narrative direction to shape the initial state of the simulation
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">KoreaPolicySim plans <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of elapsed time</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>With 100 agents, the estimated run time is about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        With 100 agents, estimated run time: {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, switching to custom mode with fewer rounds is strongly recommended for a faster preview and lower error risk -></p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-section">
            <span class="section-label">Profile Summary</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">Detailed Background</span>
            
            <!-- 人设维度概览 -->
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">Event Journey</span>
                <span class="dim-desc">Full behavioral path within this event</span>
              </div>
              <div class="dimension-card">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Relationship and interaction graph</span>
              </div>
            </div>
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generating Agent Profiles' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    if (!configTimer) {
      addLog('Starting dual-platform simulation configuration...')
      startConfigPolling()
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected an existing completed preparation. Reusing it.')
      ...
      addLog('Starting preparation status polling...')
      // 开始轮询进度
      startPolling()
      // 开始实时获取 Profiles
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          ...
          addLog('Calling the LLM to generate simulation configuration...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Simulation hours: ${data.summary.simulation_hours}`)
...
const loadPreparedData = async () => {
  phase.value = 2
  addLog('Loading existing prepared data...')
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  ├─ Simulation hours: ${res.data.summary.simulation_hours}`)
        ...
        // 配置尚未生成，开始轮询
        addLog('Configuration still generating. Starting polling...')
        startConfigPolling()
      }
    }
  } catch (err) {
