...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            ...
            if not actions:
                self.send_response(command_id, "failed", error="No valid agent found")
                return False
...
class TwitterSimulationRunner:
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(f"  - Total simulation length: {total_hours} hours")
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering wait-for-command mode. Environment remains active")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of simulation rounds (optional; truncates long runs)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately after the simulation instead of waiting for commands'
    )
